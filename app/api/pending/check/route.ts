...
const DEFAULT_TARGET_TEAMS = [
  {
    email: 'zeenathnisha2411@gmail.com',
    teamName: 'D Cure',
    paymentId: 'pay_TXqgTJDeRRRaKK',
    amount: 597.0,
    paidDate: '2026-09-04 11:46:00',
  },
  {
...
async function processPaymentFix(
  target: { email: string; teamName: string; paymentId: string; amount: number },
  ...
  if (!rows || rows.length === 0) {
    ...
    return { target, found: false, status: 'NOT_FOUND' };
  }
  ...
  // Fetch team student members
  const [memberRows]: any = await pool.query(`SELECT * FROM she_pitch_students WHERE team_id = ?`, [team.id]);
  addLog('info', `Retrieved ${memberRows ? memberRows.length : 0} student member(s) for Team ID #${team.id}.`);

  let updated = false;
  let emailSent = false;

  if (team.payment_status === 'success' && team.razorpay_payment_id === target.paymentId) {
    addLog('success', `Team ID #${team.id} ("${team.team_name}") is ALREADY marked as SUCCESS with Payment ID ${target.paymentId}.`);
  } else {
    ...
    updated = true;
    addLog('success', `Database updated! Team ID #${team.id} payment_status = 'success', razorpay_payment_id = '${target.paymentId}', amount_paid = ₹${target.amount}.`);

    // Update or insert she_pitch_payments record
    ...
    if (payRows && payRows.length > 0) {
      ...
      addLog('info', `Inserted new she_pitch_payments log record for Payment ID ${target.paymentId}.`);
    }
    ...
    try {
      const emailRes = await sendTeamConfirmationEmail({
        ...
        amountPaid: Number(target.amount),
      ...
      if (emailRes && emailRes.success) {
        emailSent = true;
        addLog('success', `Confirmation email successfully dispatched to ${team.leader_email}.`);
      } else {
        addLog('warn', `Email dispatch response: ${JSON.stringify(emailRes)}`);
      }
    } catch (mailErr: any) {
...
export async function GET() {
  ...
  try {
    ...
    for (const target of DEFAULT_TARGET_TEAMS) {
      const res = await processPaymentFix(target, addLog);
      teamResults.push(res);
    }

    addLog('success', 'Pending payment check and sync process completed for all target teams.');
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { email, team_name, payment_id, amount } = body;

    if (!email && !team_name) {
      return NextResponse.json({ error: 'Either leader email or team name is required.' }, { status: 400 });
    }
    ...
    const numAmount = Number(amount) || 597.0;

    const target = {
      email: email ? email.trim() : '',
      teamName: team_name ? team_name.trim() : '',
      paymentId: payment_id.trim(),
      amount: numAmount,
    };

    addLog('info', `Processing custom write-in payment fix for Payment ID: ${target.paymentId}...`);
    const res = await processPaymentFix(target, addLog);
    ...
    addLog('error', `Custom Payment Fix Error: ${error.message}`);
