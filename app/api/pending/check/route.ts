...
const TARGET_PENDING_TEAMS = [
  {
    teamName: 'Miss spark',
    leaderEmail: 'thejesvanisivakumar@gmail.com',
    paymentId: 'pay_TZPiMnNgHTkm4Y',
    rawPaymentId: 'TZPiMnNgHTkm4Y',
...
export async function GET(req: Request) {
  ...
  try {
      ...
      ? [
          {
            ...
            paymentId: customPaymentId ? (customPaymentId.startsWith('pay_') ? customPaymentId : `pay_${customPaymentId}`) : 'pay_TZPiMnNgHTkm4Y',
            rawPaymentId: customPaymentId || 'TZPiMnNgHTkm4Y',
            collegeName: '',
            amount: customAmount ? Number(customAmount) : 897.0,
            category: '',
          },
        ]
    ...
    for (const target of targetsToProcess) {
      ...
      const effectiveAmount = target.amount || team.amount_paid || 897.0;
    ...
    addLog('success', `🎉 Process finished. ${results.length} team(s) verified & processed.`);
...
// POST: Handles Resend Confirmation Email on demand
  ...
  try {
    ...
    if (action === 'resend_email') {
      ...
      const emailResult = await sendTeamConfirmationEmail({
        ...
        paymentId: team.razorpay_payment_id || 'pay_TZPiMnNgHTkm4Y',
        projectTitle: team.project_title,
        domain: team.domain,
        projectDescription: team.project_description,
